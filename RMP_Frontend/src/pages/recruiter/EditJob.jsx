...
export default function EditJob() {
  ...
  const addRequired = (skillId, weightage) => setForm(s => ({ ...s, requiredSkills: [...s.requiredSkills, { skillId, weightage }] }));
  const removeRequired = idx => setForm(s => ({ ...s, requiredSkills: s.requiredSkills.filter((_,i)=>i!==idx) }));
  const addPreferred = (skillId, weightage) => setForm(s => ({ ...s, preferredSkills: [...s.preferredSkills, { skillId, weightage }] }));
    ...
    <div className="p-6 max-w-3xl mx-auto">
      ...
      <form onSubmit={handleSave} className="bg-white p-6 rounded shadow space-y-4">
        ...
        <div className="flex justify-end">
          <button className="px-6 py-2 bg-indigo-600 text-white rounded" type="submit">Save</button>
        </div>
      </form>
    </div>
  );
